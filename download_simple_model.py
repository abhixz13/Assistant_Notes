...
Downloads a smaller, compatible model for testing the system
"""
...
def download_model():
    """Download a simple, compatible model."""
    
    # Simple model URL (Phi-2, much smaller and compatible)
    ...
    print("🎯 Downloading Phi-2 model (simple, compatible)...")
    print(f"📁 Model will be saved to: {model_path}")
    print(f"🌐 Download URL: {model_url}")
    print("📦 File size: ~1.5GB (much smaller, for testing)")
...
def update_config():
    """Update the configuration to use the simple model."""
    config_path = Path("config/settings.yaml")
    
    if config_path.exists():
        print("🔧 Updating configuration for simple model...")
        ...
        # Update context length for smaller model
        content = content.replace(
            'context_length: 8192',
            'context_length: 2048'
        )
...
def main():
    """Main function."""
    print("🚀 Simple Model Downloader for Testing")
    print("=" * 50)
    ...
    if model_path:
        ...
        print("\n📝 Note: This is a smaller model for testing. Once confirmed working, we can try Mixtral again.")
    else:
        print("❌ Setup failed")
